...
def _adapter_cmake(target_id: str, generation: str, projection_digest: str,
    ...
    lines.append("        # variable names sanitize node ids (hyphens -> underscores); the")
    lines.append("        # caller passes the raw node id")
    lines.append('        string(REPLACE "-" "_" _qiven_ws_pid "${provider_id}")')
    lines.append("        get_property(_qiven_ws_done GLOBAL PROPERTY \"_qiven_workspace_${_qiven_ws_pid}_materialized\")")
    ...
    lines.append("        foreach(_qiven_ws_target ${QIVEN_WORKSPACE_PROVIDER_TARGETS_${_qiven_ws_pid}})")
    ...
    lines.append('        if(NOT DEFINED QIVEN_WORKSPACE_PROVIDER_ROOT_${_qiven_ws_pid})')
    lines.append('            message(FATAL_ERROR "qiven workspace: no resolved root for ${provider_id} - the adapter does not cover this provider")')
    lines.append("        endif()")
    lines.append('        add_subdirectory("${QIVEN_WORKSPACE_PROVIDER_ROOT_${_qiven_ws_pid}}" "${CMAKE_BINARY_DIR}/qiven-workspace/${_qiven_ws_pid}" EXCLUDE_FROM_ALL)')
    lines.append('        set_property(GLOBAL PROPERTY "_qiven_workspace_${_qiven_ws_pid}_materialized" TRUE)')
