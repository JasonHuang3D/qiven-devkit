...
set(TEST_TEMPLATE_VERSION "${QIVEN_TEMPLATE_VERSION}")
set(TEST_SYNC_VERSION "${TEST_TEMPLATE_VERSION}-test")
set(TEST_ADD_VERSION "${TEST_TEMPLATE_VERSION}-add")
set(TEST_REMOVE_VERSION "${TEST_TEMPLATE_VERSION}-remove")
set(TEST_RENAME_VERSION "${TEST_TEMPLATE_VERSION}-rename")
...
function(generate_repo devkit repo_path repo_name)
    run_expect_success("${CMAKE_COMMAND}"
        -DDEVKIT_ROOT=${devkit} -DTEMPLATE_KIND=cpp-library -DDESTINATION=${repo_path}
...
function(generate_app_repo devkit repo_path repo_name)
    run_expect_success("${CMAKE_COMMAND}"
        -DDEVKIT_ROOT=${devkit} -DTEMPLATE_KIND=cpp-app -DDESTINATION=${repo_path}
        -DREPOSITORY_NAME=${repo_name} -DCMAKE_PROJECT_NAME=${repo_name}
        -DCMAKE_TARGET_NAME=${repo_name} -DCMAKE_ALIAS=qiven::example-app
...
foreach(relative IN ITEMS .clang-format .editorconfig .gitattributes CMakePresets.json AGENTS.md
        cmake/qiven/QivenBuild.cmake cmake/qiven/QivenProject.cmake cmake/qiven/QivenTarget.cmake
        cmake/qiven/QivenDependency.cmake cmake/qiven/QivenTest.cmake docs/engineering/build-system-standard.md
...
assert_contains("${repo}/.qiven/generated-state.cmake" "${TEST_TEMPLATE_VERSION}")
set(build_dogfood "${fixtures}/qiven-example-build")
run_expect_success("${CMAKE_COMMAND}" -S "${repo}" -B "${build_dogfood}" -DQIVEN_EXAMPLE_BUILD_TESTS=OFF)
run_expect_success("${CMAKE_COMMAND}" --build "${build_dogfood}")

set(app_repo "${fixtures}/qiven-example-app")
generate_app_repo("${DEVKIT_ROOT}" "${app_repo}" qiven-example-app)
foreach(relative IN ITEMS CMakePresets.json AGENTS.md cmake/qiven/QivenBuild.cmake
        cmake/qiven/QivenProject.cmake cmake/qiven/QivenTarget.cmake cmake/qiven/QivenDependency.cmake
        cmake/qiven/QivenTest.cmake docs/engineering/build-system-standard.md app/main.cpp
        CMakeLists.txt .qiven/repo.json .qiven/generated-state.cmake)
    assert_exists("${app_repo}/${relative}")
endforeach()
file(READ "${app_repo}/.qiven/repo.json" app_metadata)
string(JSON app_template GET "${app_metadata}" template)
if(NOT app_template STREQUAL "cpp-app")
    fail("generated app metadata has unexpected template kind: ${app_template}")
endif()
set(app_build_dogfood "${fixtures}/qiven-example-app-build")
run_expect_success("${CMAKE_COMMAND}" -S "${app_repo}" -B "${app_build_dogfood}" -DQIVEN_EXAMPLE_APP_BUILD_TESTS=OFF)
run_expect_success("${CMAKE_COMMAND}" --build "${app_build_dogfood}")
...
if(NOT app_state_before STREQUAL app_state_after)
    fail("cpp-app no-op sync changed generated state")
endif()
...
file(APPEND "${devkit_copy}/templates/native-common/managed/.editorconfig.in" "\n# fixture-template-update\n")
file(READ "${devkit_copy}/templates/cpp-library/managed-files.cmake" manifest)
string(REPLACE "${TEST_TEMPLATE_VERSION}" "${TEST_SYNC_VERSION}" manifest "${manifest}")
...
assert_contains("${repo}/.qiven/generated-state.cmake" "${TEST_SYNC_VERSION}")
...
if(NOT synced_repo_version STREQUAL "${TEST_SYNC_VERSION}" OR NOT QIVEN_TEMPLATE_VERSION STREQUAL synced_repo_version)
...
file(APPEND "${devkit_copy}/templates/native-common/managed/.clang-format.in" "\n# devkit managed edit\n")
file(APPEND "${devkit_copy}/templates/native-common/managed/.gitattributes.in" "\n# must-not-apply\n")
...
file(READ "${add_devkit}/templates/cpp-library/managed-files.cmake" add_kind_manifest)
string(REPLACE "${TEST_TEMPLATE_VERSION}" "${TEST_ADD_VERSION}" add_kind_manifest "${add_kind_manifest}")
file(WRITE "${add_devkit}/templates/cpp-library/managed-files.cmake" "${add_kind_manifest}")
file(READ "${add_devkit}/templates/native-common/managed-files.cmake" add_manifest)
...
file(WRITE "${add_devkit}/templates/native-common/managed-files.cmake" "${add_manifest}")
file(WRITE "${add_devkit}/templates/native-common/managed/docs/engineering/new-managed.md.in" "# Newly managed\n")
...
file(APPEND "${add_devkit}/templates/native-common/managed/.editorconfig.in" "\n# collision-atomicity\n")
...
file(READ "${remove_devkit}/templates/cpp-library/managed-files.cmake" remove_kind_manifest)
string(REPLACE "${TEST_TEMPLATE_VERSION}" "${TEST_REMOVE_VERSION}" remove_kind_manifest "${remove_kind_manifest}")
file(WRITE "${remove_devkit}/templates/cpp-library/managed-files.cmake" "${remove_kind_manifest}")
file(READ "${remove_devkit}/templates/native-common/managed-files.cmake" remove_manifest)
string(REPLACE "    .gitattributes\n" "" remove_manifest "${remove_manifest}")
file(WRITE "${remove_devkit}/templates/native-common/managed-files.cmake" "${remove_manifest}")
...
file(READ "${old_devkit}/templates/native-common/managed-files.cmake" old_manifest)
string(REPLACE "    tools/delete-all-branches-but-main.cmd\n)" "    tools/delete-all-branches-but-main.cmd\n    docs/engineering/old-name.md\n)" old_manifest "${old_manifest}")
file(WRITE "${old_devkit}/templates/native-common/managed-files.cmake" "${old_manifest}")
file(WRITE "${old_devkit}/templates/native-common/managed/docs/engineering/old-name.md.in" "# Renamed content\n")
...
file(READ "${rename_devkit}/templates/cpp-library/managed-files.cmake" rename_kind_manifest)
string(REPLACE "${TEST_TEMPLATE_VERSION}" "${TEST_RENAME_VERSION}" rename_kind_manifest "${rename_kind_manifest}")
file(WRITE "${rename_devkit}/templates/cpp-library/managed-files.cmake" "${rename_kind_manifest}")
file(READ "${rename_devkit}/templates/native-common/managed-files.cmake" rename_manifest)
string(REPLACE "docs/engineering/old-name.md" "docs/engineering/new-name.md" rename_manifest "${rename_manifest}")
file(WRITE "${rename_devkit}/templates/native-common/managed-files.cmake" "${rename_manifest}")
file(RENAME "${rename_devkit}/templates/native-common/managed/docs/engineering/old-name.md.in" "${rename_devkit}/templates/native-common/managed/docs/engineering/new-name.md.in")
...
include("${CMAKE_CURRENT_LIST_DIR}/build-system-test.cmake")
